...
**「フォールバックなしを優先」** にしておくと、CUDA の確保が溢れたときにフリーズではなく
CUDA OOM で即座に失敗します。ただしこれは **CUDA の確保にしか効きません**。推論サーバが
VRAM を確実に取れるようになる代わりに、しわ寄せが描画側へ移ります（次項）。

##### 描画側（dwm）が VRAM を握っている場合

サーバ側を削っても空きが増えないときは、GPU を奪っているのが描画側かもしれません。
`nvidia-smi` は GeForce だと**プロセス別の使用量を報告しない**ので、WDDM の
パフォーマンスカウンタから読みます。

実測（RTX 4070 / 12282MiB、3840×2160 の 2 画面・HDR 有効、可視ウィンドウ 29、
ログオンから 4 日連続）:

| プロセス | 専用 VRAM |
| --- | --- |
| llama-server | 6725 MiB |
| **dwm** | **5464 MiB** |
| 2 つの合計 | **12189 MiB**（物理 12282MiB をほぼ使い切り） |

HDR を有効にすると dwm のコンポジションが FP16（8 バイト/px）になり、4K サーフェス 1 枚が
33MiB → **66MiB** になります。ウィンドウを開くほど積み上がり、**自力では戻りません**。
この状態で推論が GPU を飽和させると WDDM が dwm のサーフェスをシステムメモリへ追い出し、
以後は描画のたびに PCIe 越しで戻すことになるため、画面更新もマウスカーソルも止まります。
**推論自体は 20 tok/s で正常に走っているのに操作できなくなる**のがこの症状の見分け方です
（サーバログの `tg = ... t/s` が正常なら、詰まっているのは描画側）。
...
返ったかがその場で分かります。`taskkill` が拒否される環境では、解像度を変えて戻す
（モードセットでサーフェスが作り直される）か `Win+L` でロックして再サインインする方法を
案内します。日本語コメントを含むため、このスクリプトは UTF-8(BOM 付き)で保存しています
（BOM が無いと PowerShell 5.1 が cp932 として読んで壊れます）。

恒久的に切り離すなら、モニタを iGPU 出力へ移して**デスクトップ描画を内蔵 GPU に任せる**のが
確実です。dwm のぶんが丸ごと空くので、HDR を維持したまま推論サーバと TTS の両方を
GPU に載せられます。
