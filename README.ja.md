...
- **事実台帳（集計）**: 往復から「誰が・誰に・何を・どうした」を抽出し `facts` テーブルへ正規化。列挙は検索ではなく `SELECT DISTINCT` なので**全件が返る**
  ...
  - `occurred` は出来事そのものの時期。「1年前」「去年の夏」「昨日」を往復の `ts` を基準日にして解決し、粒度つきの前方一致文字列（`2025` / `2025-07` / `2025-07-24`）で持つ。原文の言い方は `time_hint` に残して提示にも出す（`ts` は**その話をした日**にすぎない）
...
**ツール**
- `tools/build_fact_ledger.py`: 既存ログから台帳を一括構築（`--rule-only` でLLM不使用、`--dry-run` で確認、中断しても未抽出分から再開、`--list --modality plan` で予定として記録された分だけ確認）
