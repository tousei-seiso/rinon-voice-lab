...
# そのうち「に」が場所だけを指す動詞。「〜に行く」の「に」は**目的**も表すため
# （「気分転換に行く」「任務に行く」「買い物に行く」）、場所らしさを見ないと抽象名詞が
# 場所として台帳に入る（実測: 30 件中 12 件がこの形だった）。
# 「会う」は「友達に会った」のように「に」が相手を指すので、この検査からは外す。
_PLACE_VERBS = {"行く", "登る", "泊まる"}
# 場所・出来事として通る客体の目印。カタカナだけの語（固有名詞: ネパール／マリンタワー）と
# 場所カテゴリの語彙も通す。ここに載っていない語を落とす副作用より、抽象名詞が
# 「行った場所」として並ぶ害の方が大きい（「へ」を伴う場合は方向が確定するので検査しない）。
_PLACE_LIKE_RE = re.compile(
    r"駅|空港|港|局|店|屋|館|園|寺|神社|宮|城|山|川|河|海|島|湾|浜|岸|滝|湖|池|泉|洞窟|"
    r"町|市|村|区|県|都|府|道|街|通り|橋|坂|宿|部屋|家|実家|会社|事務所|工場|研究所|基地|"
    r"病院|医院|クリニック|学校|大学|校|公園|温泉|銭湯|銀行|市場|会場|ホール|球場|"
    r"ホテル|旅館|カフェ|喫茶|レストラン|食堂|センター|ルーム|モール|デパート|百貨店|"
    r"スタジアム|タワー|ビル|マンション|展望台|屋上|庭|畑|森|林|遺跡|"
    r"地|所|場|大会|祭|ライブ|イベント|試合|コンサート|花火|旅行"
)
...
# 客体の頭に付く副詞（「また横浜」→「横浜」）。時間表現と同じ経路で剥がす。
_OBJECT_ADVERB_PREFIX_RE = re.compile(
    r"^(?:また|もう|ちょっと|やっぱり|やはり|やっと|すぐ|早速|さっそく|まず|とりあえず|"
    r"結局|ついに|一緒に|そのまま)(?:の|は|も|に|で|、)?"
)
...
def _clean_object(text: str) -> str:
    ...
    for _ in range(2):
        ...
        trimmed = _OBJECT_ADVERB_PREFIX_RE.sub("", trimmed, count=1).strip()
        if trimmed == body:
            break
        body = trimmed
...
def _find_verb_object_pairs(text: str) -> list[tuple[str, str, str, str, str]]:
    ...
    for verb, pattern in _VERB_PATTERNS:
        ...
        # 助詞も取っておく（「に」だけは場所以外＝目的も表すので、後で場所らしさを見る）。
        regex = re.compile(
            r"(?P<obj>[^、。！？\n\s「」『』]{1,20}?)"
            + r"(?P<particle>"
            + particle
        ...
        while True:
            ...
            if _ANY_VERB_RE.search(match.group("gap")):
                continue
            obj = _clean_object(match.group("obj"))
            if obj and verb in _PLACE_VERBS and match.group("particle") == "に":
                # 「〜に行く」の「に」は目的も表す。場所らしくない客体は、その行為の
                # 場所ではない（「気分転換に行く」「任務に行く」「買い物に行く」）。
                # 「へ」は方向しか表さないので検査しない。
                if not (
                    _PLACE_LIKE_RE.search(obj)
                    or re.fullmatch(r"[ァ-ヴー]+", obj)
                    or _guess_category(obj, verb) == "場所"
                ):
                    obj = ""
            if not obj:
                # 助詞の直前が客体になれない語（時間表現・形式名詞・場所でない語）だった
                # だけで、本当の客体はこの先にあることが多い（「1年前に多摩川の花火大会に
                # 行った」「去年の夏に江ノ島へ行った」）。その語と助詞の分だけ進めて同じ
                # 動詞を探し直す。動詞ごと読み飛ばすと客体を丸ごと失う。
                position = match.end("obj") + 1
                continue
